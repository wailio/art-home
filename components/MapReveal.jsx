  ...
  return (
    <div ref={wrapRef} style={{ position: "relative", width: "100%", aspectRatio: "16/10" }}>
      <svg width="100%" height="100%" viewBox="0 0 480 300" style={{ position: "absolute", inset: 0, zIndex: 3, pointerEvents: "none" }}>
        <rect x="4" y="4" width="472" height="292" fill="none" stroke="#5a3d12" strokeWidth="2" strokeDasharray="1528" strokeDashoffset={on ? 0 : 1528} style={{ transition: "stroke-dashoffset 1.1s ease-out" }} />
      </svg>
      ...
      <div style={{ position: "absolute", left: "50%", top: "44%", zIndex: 4, pointerEvents: "none", transform: on && pinVisible ? "translate(-50%,0)" : "translate(-50%,-30px)", opacity: pinVisible ? 1 : 0, transition: "transform 0.6s cubic-bezier(0.34,1.56,0.64,1) 0.75s, opacity 0.4s ease" }}>
        <span style={{ position: "absolute", left: "50%", bottom: 0, width: 14, height: 14, marginLeft: -7, border: "1.5px solid #5a3d12", borderRadius: "50%", opacity: pinVisible ? 1 : 0, animation: pinVisible ? "mapPinPulse 1.6s ease-out 1.35s infinite" : "none" }} />
        <MapPin size={34} color="#5a3d12" fill="#5a3d12" strokeWidth={1.5} />
      </div>
